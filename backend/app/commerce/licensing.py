...
where ``body`` is base64url(JSON) and ``signature`` is an HMAC-SHA256 over the
body keyed by ``LENGRVIS_LICENSE_SIGNING_KEY``. Only the standard library is
used so this works in a fully offline build.
...
Note: HMAC is symmetric, so the deployment that verifies also holds the signing
secret. That is acceptable for offline self-hosted Team installs; a SaaS
distribution should swap in asymmetric (public-key) verification here.
"""
...
LICENSE_SIGNING_KEY_ENV_VAR = "LENGRVIS_LICENSE_SIGNING_KEY"
...
def sign_license(payload: dict[str, Any], signing_key: str) -> str:
    """Produce a ``<body>.<signature>`` license token (test/admin helper)."""
    if not signing_key:
        raise LicenseError("A signing key is required to sign a license")
    body = _b64url_encode(json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8"))
    return f"{body}.{_signature(body, signing_key)}"
...
def parse_license(token: str, signing_key: str) -> License:
    """Verify the token signature and decode it (raises :class:`LicenseError`)."""
    if not token or not token.strip():
        raise LicenseError("License token is empty")
    if not signing_key:
        raise LicenseError("No license signing key configured", code="license_signing_key_missing")
    body, _, signature = token.strip().partition(".")
    if not body or not signature:
        raise LicenseError("License token is malformed")
    expected = _signature(body, signing_key)
    if not hmac.compare_digest(expected, signature):
        raise LicenseError("License signature does not match", code="license_signature_mismatch")
...
def verify_license(token: str, signing_key: str, *, now: datetime | None = None) -> License:
    """Parse and ensure the license is currently active (raises on expiry)."""
    license_ = parse_license(token, signing_key)
...
def _read_signing_key() -> str:
    return (os.getenv(LICENSE_SIGNING_KEY_ENV_VAR) or "").strip()
...
def load_license(settings: Any | None = None, *, now: datetime | None = None) -> License | None:
    ...
    if not token:
        return None
    signing_key = _read_signing_key()
    if not signing_key:
        logger.warning("License token present but %s is not set; ignoring license.", LICENSE_SIGNING_KEY_ENV_VAR)
        return None
    try:
        license_ = parse_license(token, signing_key)
...
def apply_licensed_plan(settings: Any, *, now: datetime | None = None):
    ...
    try:
        ...
        try:
            setattr(settings, "plan", plan.value)
