...
def test_remote_lan_client_can_pair_but_not_use_desktop_apis(monkeypatch, tmp_path):
    monkeypatch.setenv("MARVIS_DATA_DIR", str(tmp_path))
    monkeypatch.delenv("MARVIS_ALLOW_LAN_DESKTOP_API", raising=False)
    db.init_db()
    client = TestClient(app, client=("192.168.1.22", 50100))

    assert client.post("/api/pair/code").status_code == 403
    code_response = client.post("/api/pair")
