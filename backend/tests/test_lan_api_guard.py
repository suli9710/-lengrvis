...
def test_browser_host_websocket_requires_desktop_authorization(monkeypatch, tmp_path):
    ...
    with remote.websocket_connect(
        "/api/ws/browser-host",
        headers={"Sec-WebSocket-Protocol": "mavris.desktop.token.desktop-secret"},
    ) as websocket:
...
def test_remote_lan_desktop_get_requires_desktop_token_when_enabled(monkeypatch, tmp_path):
    monkeypatch.setenv("MARVIS_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("MARVIS_ALLOW_LAN_DESKTOP_API", "1")
    monkeypatch.setenv("MAVRIS_DESKTOP_API_TOKEN", "desktop-secret")
    ...
    assert loopback.get("/api/tasks").status_code == 200
