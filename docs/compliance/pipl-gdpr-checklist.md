...
> 最后更新：2026-06-24。法务复核：**未完成**（阻断项，见底部）。
...
- [x] **本机一键删除入口**：`POST /api/system/privacy/erase-local-data`（需显式确认词 `erase-local-data`）。
  ...
  - 证据：`backend/tests/test_privacy_erase.py` 与桌面 IPC 安全冒烟测试。
- [x] **桌面 UI 删除入口**：设置页提供“本机数据与隐私”，要求输入 `删除本机数据`，再通过 Electron 原生确认；渲染层不能直接提交后端确认词。
...
| 义务 | 状态 | 说明 |
| --- | --- | --- |
| 告知同意（隐私政策与 EULA 展示、同意记录） | ⚠️ 部分完成 | 桌面首启已集成两份文档的独立勾选、版本与时间戳记录，版本变化会触发对应文档重新同意；政策/EULA 仍是法务未定稿草案，安装器展示尚未完成 |
...
1. 隐私政策与 EULA 法务定稿；验证首启同意门禁与发布包法律资源，补齐安装器展示要求（清单 #5）。
2. 完整用户数据导出/导入（清单 #36）。
3. 日志自动清理、支持/隐私责任人、保留周期与演练证据。
