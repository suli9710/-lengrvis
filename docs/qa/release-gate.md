...
Record the emitted `.tmp\release-evidence-packet\...\release-evidence-packet.redacted.json` and `.tmp\release-evidence-packet\...\release-evidence-packet.redacted.md` files as a packet index, not as a pass. The packet summarizes the latest mobile LAN/WSS preflight redacted summary when present, the latest portable first-screen/read-only/natural-language status-log summary when present, the current Ollama/local-model contract count, latest local-model clean-machine handoff template status when present, latest natural-language result-quality review packet status when present, diagnostics `support_package_redaction.external_review` expectations with `public_safe=false`, and Settings local-model smoke artifact paths. It also emits `summary.release_ready=false`, `summary.claimable_release_signoff=false`, `summary.release_readiness_blocker_count=7`, and `release_readiness_blockers` entries for candidate-bound agentic threat-model review, clean-machine local-model evidence, mobile real-device LAN/WSS preflight, installable Android remote-control evidence, natural-language result-quality sign-off, diagnostics external public-safety review, and release-candidate handoff. Treat those blocker entries as the beginner-facing missing-evidence dashboard, not as waivers. Portable status-log coverage is packaged window/backend/local-only diagnostics plus command-dock submission/task-evidence coverage only. It is not candidate-bound threat-model approval, not clean-machine local-model readiness, not true local model install/start/pull evidence, not real-device mobile evidence, not external diagnostics public-safety approval, not completed task-result sign-off, not natural-language result-quality sign-off, and not release-candidate sign-off. Product/API task explain surfaces carry completed-result evidence separately through `completion_evidence`: the portable smoke records explain `completion_evidence.level` and `result_verified`, but `submission`, `task_created`, and `visible_progress` levels are not completed-result evidence; only `completion_evidence.level=completed_result` with `result_verified=true` may be called completed-result evidence, and that is still not result quality review, RC sign-off, or release sign-off. `completion_evidence.signoff` remains false. Treat this helper as a handoff template and contract summary only: it cannot replace candidate-bound threat-model review, human review of the actual exported diagnostics contents, clean-machine validation, result-quality sign-off, or RC sign-off.
...
Use `npm run evidence:local-model-template` as a template-only helper, not clean-machine pass evidence, not true install/start/pull/task-smoke evidence, and not release-candidate sign-off; all pass/signoff fields remain false until separate manual evidence is recorded. Use `npm run evidence:clean-machine-verify -- --require-local-model` only after the reviewed clean-machine JSON is filled from real install/start/pull/task-smoke artifacts.
...
npm run evidence:diagnostics-verify -- --evidence build/diagnostics-external-review-evidence-reviewed.json
```
...
python scripts\verify_diagnostics_external_reviewed_evidence.py --evidence build/diagnostics-external-review-evidence-reviewed.json
```

Record the emitted `.tmp\diagnostics-external-review\...\diagnostics-external-review.redacted.json` and `.tmp\diagnostics-external-review\...\diagnostics-external-review.redacted.md` files as a diagnostics review template only. The helper can organize redacted checklist fields and keep `public_safe=false`, `external_sharing_allowed=false`, and `claim_allowed=false`, but it does not review the actual exported package contents and is not public-safe/signoff, clean-machine validation, RC signoff, release signoff, or permission to publish the package. Even when the helper exits 0, `review_scope.automated_redaction_template=true`, `review_scope.actual_package_content_review_completed=false`, `summary.review_fields_complete=false`, `summary.external_sharing_blocked=true`, `summary.separate_human_content_review_required=true`, `claim_controls.public_safe_approval_created=false`, and release packets may record `manual_content_review_only_remaining` as true; the checklist must still carry the actual exported package path label, reviewed logs, path labels, task traces, model traces, device identifiers, reviewer/timestamp fields, and blocked reason before any separate human content-review artifact can be considered. The verifier accepts only signed `diagnostics-external-review-evidence-reviewed` JSON with passed manual checks, an explicit non-public sharing decision, a redacted package label, and an explicit UTC timestamp; it rejects the checklist template and cannot authorize `public_safe=true` or external sharing. In strict delivery, the strict `diagnostics-evidence` stage is the final machine gate for the signed reviewed artifact, so RR-P0-005 should be treated as machine-chain-ready only when that path is wired and the remaining blocker is the human content review of the actual exported package.
...
`privacy.strict_state_machine: true` and `llm.allow_mock_fallback: false`
explicit for a candidate build. Never add secrets to that example file.
...
The GitHub publish workflow is manual-only and runs the same strict preflight,
signing requirement, and `delivery:rc` verdict before it can create or upload a
release. A tag alone is never publishing authorization.
...
- Extension security verification passes: `npm run security:extensions` completes the IPC policy/openExternal smoke, targeted Skill/MCP/settings security tests, Skill/MCP release-profile supply-chain gate, and records or references the `extension-security-gate` evidence artifact.
...
- Default Windows artifacts must stay small: `scripts\verify_packaging.ps1` rejects the default portable zip or self-extracting exe when either is larger than 500MB. Default portable artifacts must not contain `resources\ollama`, `resources\ollama-models`, or `resources\ollama-bundle-manifest.json` in the portable directory or portable zip; local models are prepared on first run by downloading them on demand.
...
- Structural verification also asserts the backend capability manifest: `dist\backend-capabilities.json` and `resources\backend\backend-capabilities.json` (also a portable-zip entry) must exist, parse, use schema `lengrvis-backend-capabilities/v1`, and carry every optional capability key defined in `backend/build_backend.py` (`docling`, `unstructured`, `paddleocr`, `pytesseract`, `playwright`, `pywhispercpp`). Pass `-RequiredBackendCapabilities docling,paddleocr` (through `scripts\build_all.ps1` or directly) to fail the gate when a release-required capability was not bundled by the build machine. This prevents different build machines from silently producing a `backend.exe` with fewer capabilities.
...
- If a special offline Ollama release is being prepared, rerun verification with `scripts\build_all.ps1 -VerifyOnly -RequireBundledOllama -RunExecutableSmoke` and confirm the runtime, models, bundle manifest summaries, and backend runnable smoke match the packaged files. Treat that as a separate offline model or enterprise package; it is not the default under-500MB consumer installer.
...
- Record `release:check` as passing only when the whole command exits 0. If `qa:gate`, `supply-chain:verify`, `audit:deps`, `security:secrets`, `security:extensions`, `release:safety`, structural checks, runnable backend smoke, or signature verification pass before a later failure, record those as partial sub-gate evidence and keep the artifact gate failed. Record commercial-loop evidence under `delivery:paid-launch`, not under no-sale RC evidence.
...
| Local/hybrid model evidence | Settings shows quick/privacy/hybrid model boundary, recommended model, download size, hardware status, speed estimate, and the privacy failure path that does not auto-fall back to cloud. The default installer must stay under 500MB and must not bundle Ollama/model resources; local-model clean-machine evidence should record first-run setup-plan,联网 install/start/pull, model/runtime/version/status, and then断网 verification for at least two of system diagnostics, file search, and simple local summary. `.\scripts\collect_local_model_clean_machine_evidence_template.ps1` may be used to capture the redacted artifact/build/profile, runtime/model/version/status, install/start/pull/task-smoke outcome or blocked reason fields, but its `NOT_REAL_LOCAL_MODEL_INSTALL_START_PULL_PASS` output is only a handoff template. Vite-preview Settings DOM smoke and Ollama backend contract tests alone are not sufficient. The recorded Settings local-model smoke provides Vite/mock visual regression at 1366px desktop and 900px narrow desktop widths, and historical Ollama backend targeted tests record `53 passed`; still record packaged or manual evidence before calling the release-candidate Settings UX or true local model install/start/pull/task-smoke signed off. |
