...
- Seal the completed JSON with `npm run evidence:android-real-device-seal` while the protected `LENGRVIS_RELEASE_EVIDENCE_HMAC_SECRET` and immutable candidate context are present. The HMAC key must be generated from a cryptographically secure source, contain at least 32 bytes, and must not be a human password or development/CI placeholder. Android evidence uses `reviewed-evidence-hmac-sha256/v2`, which includes the full SHA-256 signing-key fingerprint in the signed payload; strict validation also recomputes that fingerprint from the configured key and rejects a re-signed false label. A template or an unsigned JSON is not strict-gate input.
