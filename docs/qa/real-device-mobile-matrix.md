...
- `npm --prefix mobile run gate:android-connected-lan-tls` is a release/evidence-only connected Android regression for wrong-pin failure, pinned HTTPS pairing, and approval WSS when a configured device/emulator and LAN TLS backend are available. Attach it as supporting trust-path evidence, not as a replacement for reviewed screenshots/logs or the strict Android evidence JSON.
...
- The generated `android-real-device-evidence.redacted.template.json` is not the strict-gate input until a reviewer fills a separate reviewed JSON with `real_device_result=passed`, `review.status=reviewed_passed`, redacted `device`, `transport`, `certificate`, `evidence_artifacts_redacted`, a complete `evidence_artifact_manifest`, and all five immutable `candidate` fields, and every required check below is backed by phone/emulator artifacts.
- Seal the completed JSON with `npm run evidence:android-real-device-seal` while the reviewer-only `LENGRVIS_REVIEWED_EVIDENCE_PRIVATE_KEY`, its matching `LENGRVIS_REVIEWED_EVIDENCE_PUBLIC_KEY`, and immutable candidate context are present. Generate the pair with `npm run evidence:keypair -- --private-key-output <private-path> --public-key-output <public-path>`, import the private value into reviewer-controlled secret storage, and expose only the public value to release verification. Key generation does not seal evidence and is not release sign-off. Android evidence uses `reviewed-evidence-ed25519/v3`; the full public-key SHA-256 fingerprint, candidate identity, artifact manifest, and artifact hashes are inside the canonical signed payload. A template, legacy HMAC evidence, or an unsigned JSON is not strict-gate input.
- Strict `npm run android:release-gate -- -ArtifactPath "<qa apk path>" -RealDeviceEvidencePath "<sealed android evidence json>" -ExpectedSignerCertificateSha256 "<controlled release certificate sha256>" -RequireCandidateBinding` may be recorded as Android gate evidence only after Android SDK v2/v3 signature verification, controlled signer matching, package/version inspection, candidate-bound `app.provenance`, and this matrix's phone/emulator artifacts all pass.
...
The completed draft must contain `evidence_artifact_manifest.version=sha256-manifest/v1`. Its entries bind each redacted label to `kind`, SHA-256, and positive byte size; labels must exactly match `evidence_artifacts_redacted`. At minimum include `device_screenshot`, `device_video`, `backend_log`, `mobile_log`, and `adb_install_status`. Manifest labels are filenames or opaque redacted labels only—never private paths or URLs.
