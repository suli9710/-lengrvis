...
This matrix defines the manual Android or emulator evidence required before a release/demo may claim real mobile pairing, LAN TLS, remote screen, or remote input. Existing local smoke tests support this matrix but do not satisfy it.
...
| RD-004 | Approval WebSocket over WSS | With paired device, create an approval on desktop and receive it on mobile. Approve and reject one benign request. | Mobile screen/video plus backend/audit log note; no token in URL artifacts. | Backend and local smoke covered; no real-device WSS evidence. |
...
| RD-016 | Artifact redaction | Inspect screenshots, videos, backend logs, app logs, and proxy traces from the run. | No raw mobile token, grant token, pairing code, private path, device name, or task secret appears in shareable artifacts. | Automated redaction tests exist for some payloads; run-specific artifact review still required. |
