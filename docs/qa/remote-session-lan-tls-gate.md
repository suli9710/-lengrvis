...
Historical automated support evidence for the 2026-06-09 development workspace includes the combined `python -m pytest backend/tests/test_mobile_pairing.py backend/tests/test_remote_desktop.py -q` targeted run, plus passing mobile/desktop targeted smokes/typechecks in that integration. Scheduler/preflight targeted checks are support-only development notes unless their exact command and run log are attached; do not cite an unbound count from this gate. These results strengthen repository contract coverage only; the release pass rule below still requires real-device or documented emulator HTTPS/WSS evidence.

## Audit Disposition

| Audit claim | Disposition | Repository evidence | Remaining gap |
| --- | --- | --- | --- |
| Remote WebSockets may lack scoped auth. | Mostly not true for current backend. | Historical backend mobile+remote targeted evidence covers remote screen disabled-by-default, `remote:view` scope acceptance, approval-scope rejection, remote view/input cross-scope rejection, query-token rejection, device revoke close, token expiry close, and remote desktop disabled close. `backend/app/api/routes_remote.py` requires mobile WebSocket subprotocol tokens and scope-specific decode. | Real Android over LAN/WSS must still prove the same behavior outside TestClient. |
| Remote input grant revoke/expiry may not be enforced. | Mostly not true for automated backend/client contract coverage. | Historical backend mobile+remote targeted evidence covers connected and idle `/ws/remote/input` close after grant revoke, grant expiry, token expiry, cross-scope rejection, remote desktop disable, and text/key input event approval contracts. `mobile/scripts/remote-input-grant-smoke.cjs` covers claim, subprotocol use, wrong token rejection, revoke, expiry, and active-grant approval mismatch blocking against local/source behavior. | A real mobile session must show visible read-only fallback after revoke/expiry and no further input events. |
| Remote-input approval may be approved from a stale or different mobile grant. | Mostly not true for current client contract coverage. | Mobile approval list/detail/client paths now require a remote-input approval to match the current mobile session's active binding before claiming a grant token or submitting an approval decision. Shareable evidence uses the HMAC `binding_ref` or a redacted active-grant label; raw `deviceId`/`grantId` correlation stays local-only. The current remote-input smoke covers missing, mismatched, and matching active-grant cases. | A real phone/emulator run still needs screenshots/log notes showing the active grant, approval prompt, decision, revoke/expiry, and no stale input after grant changes. |
| Mobile tokens may be over-scoped or reusable for wrong endpoints. | Mostly not true for current backend tests. | `backend/app/security/mobile_jwt.py` separates `mobile:approval`, `remote:view`, and `remote:input`; grant tokens require `source=remote_input_grant` and `grant_id`. Historical backend mobile+remote targeted evidence covers remote-input-only token rejection for mobile approval WebSocket and mobile API resources. | Real-device evidence still needs token expiry/revoke UI behavior. |
...
If the Android release gate is used for the same candidate, treat its preflight output as setup evidence only. The strict Android gate output can support this LAN/TLS gate only when it references an installable APK and a reviewed real-device evidence JSON whose artifacts cover the same HTTPS/WSS, certificate trust, remote screen/input, revoke/expiry, and redaction requirements below.
...
2. Real-device or documented emulator layer:
   ...
   - Remote input state: read-only default, grant received, remaining time visible, input connected, click/text/key input approval records, zoom/pan/landscape usability, active remote-input approval source matching the current mobile public `binding_ref` or redacted active-grant label, mobile end-control, desktop revoke, grant expiry, and no input after revoke/expiry. Raw `deviceId`/`grantId` correlation stays in local-only reproduction notes.
