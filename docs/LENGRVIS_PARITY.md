...
As of the latest 2026-06-09 development handoff, targeted evidence in this dirty workspace records desktop/mobile typecheck passing, mobile token/task-companion/remote-input smokes passing, desktop mobile-pairing QR and remote-input grant smokes passing, backend mobile+remote targeted combined run `132 passed`, and `git diff --check` exit 0 with LF-to-CRLF working-copy warnings only. Scheduler/preflight targeted checks are support-only development notes unless their exact command and run log are attached; do not cite an unbound `9 passed` count from this roadmap. The mobile/remote evidence includes active-grant remote-input approval guards and backend WebSocket contracts for click, text, and key input at source/client/backend-contract level: a phone approval must match the current mobile session's active remote-input binding before the client claims or submits a remote-input decision. Shared evidence should use the HMAC `binding_ref` or a redacted active-grant label; raw `deviceId`/`grantId` correlation stays in local-only reproduction notes. The previously tracked P1 review findings plus this round's public task text bare-filename/hidden-prompt leakage, realtime raw malformed-message sampling, mobile QR transport metadata bypass, and remote-input active-grant mismatch findings are closed at source/client/backend-contract level. This is useful productization evidence, but it is not clean-machine RC sign-off, real phone/WSS proof, true local model install/start/pull proof, completed task-result sign-off, result-quality sign-off, or external diagnostics human review.

Evidence boundary for parity claims: helper and preflight commands keep the product honest, but they are not market-facing proof by themselves. Android parity requires an installable APK plus reviewed phone/emulator WSS evidence; local-model parity requires clean-machine install/start/pull/task-smoke evidence or an explicit blocked reason; diagnostics parity requires actual exported package content review before any external sharing; RC/release parity requires release-owner approval after exact gate logs, manual P1 checks, waivers, and residual risks are attached. Until those artifacts exist, describe the product as source/contract supported and demo-bound, not top-tier release-ready.
...
| Agent 编排与工程能力 | Orchestrator、Planner、Supervisor、SafetyReview、OSExecutionEngine、Memory 形成实质编排；File/Document/Computer/App/Browser/Search 是 domain shell agents；38 个 prompt 模板外部化；step graph 支持并行；任务 explain/list/detail/replay/timeline 已带 beginner-safe `result_quality`/`completion_evidence` 边界。证据：`backend/app/agents/*`、`backend/app/orchestration/*`、`backend/app/llm/prompts/`、`backend/app/services/task_explain_service.py` | 任务时间线、Task Workspace、信任清单可以展示计划、进度、审批、结果状态。证据：`desktop/src/renderer/components/TaskTimeline.tsx`、`desktop/src/renderer/features/office/OfficeScene.tsx` | 领域 shell agent 的失败恢复、工具选择质量、用户可读解释还需要从“能执行”提升到“稳定完成任务”；`result_quality.signoff` 仍保持 false，不能写成自然语言成果质量签收 | 宣称每个领域都有独立自主推理 Agent；当前不应这样对外表达 |
...
| 移动 companion 与远程桌面 | Android companion 有配对、内置 `expo-camera` QR 扫码入口、粘贴/手动 fallback、JWT、审批列表/详情、批准/拒绝、任务状态、通知隐私；后端有远程屏幕流、输入 WebSocket、token scope、remote input approval；当前 backend mobile+remote targeted combined run 记录 `132 passed`，active-grant guard 由 backend 字段断言与 mobile/desktop remote-input source/client smoke 共同支撑，click/text/key 远控输入审批契约由 backend/client 目标证据支撑。证据：`mobile/src/screens/*`、`mobile/scripts/mobile-token-smoke.cjs`、`backend/app/api/routes_mobile.py`、`backend/app/api/routes_remote.py`、`backend/app/services/mobile_pairing_service.py` | 手机只读看屏幕、远程输入短授权、文字/常用按键输入、屏幕缩放/平移/横屏查看、结束接管、LAN/TLS 提示可演示；桌面 QR 生成、移动端 PairScreen 扫码源码入口、payload parser、active-grant guard 和后端 LAN TLS ready/misconfigured metadata 口径有自动/source smoke 证据。证据：`mobile/src/screens/RemoteScreen.tsx`、`docs/qa/e2e-acceptance-matrix.md`、`backend/tests/test_lan_transport_security.py` | 真实手机/模拟器扫码配对、网络穿透、锁屏权限、延迟、实际 HTTPS/WSS 服务路径、设备侧证书信任链、键盘弹出/横竖屏/缩放平移手感、移动端继续/发起任务仍需产品化；源码/backend/client smoke 不能替代真实设备证据 | iOS companion、手机完整接管 PC、PC 操作手机 App |
...
| 打包、发布与 QA gate | `qa:gate`、`release:check`、`release:quick`、runnable backend smoke、portable launcher/backend diagnostics smoke、portable renderer DOM read-only entry smoke、portable natural-language `/api/runs` submission/task-evidence smoke、dependency lock verification、release safety gate 已整理；当前 dirty workspace 另有 backend mobile+remote targeted combined `132 passed`、desktop/mobile typecheck、mobile/desktop targeted smokes，以及 prior + latest P1/P2 review findings closed at contract level。Scheduler/preflight checks must carry their exact command/log before any count is cited. 证据：`package.json`、`docs/qa/release-gate.md`、`docs/qa/e2e-acceptance-matrix.md`、`scripts/portable_first_screen_smoke.ps1` | Windows artifact structural + backend runnable smoke + portable 窗口/health/local-only diagnostics + packaged renderer DOM read-only click path + packaged natural-language command dock submission/task evidence 可演示；special offline Ollama release 有显式 `-RequireBundledOllama` 验证入口 | 真实机器候选启动、packaged GUI 自然语言任务进度/结果、completed task-result sign-off、result-quality sign-off、macOS DMG、Android 分发和真实 LAN/WSS 证据仍需人工或平台验收；当前 packaged natural-language smoke 只记录 submission + backend read-only/system diagnostics task evidence，不证明完整任务结果、clean-machine 或 RC 签收 | iOS 分发、自动更新、完整 crash/diagnostic pipeline |
...
- Focused mobile/remote evidence: `python -m pytest backend/tests/test_mobile_pairing.py backend/tests/test_remote_desktop.py -q` (current combined run: `132 passed`; source/client smokes are supporting evidence, not real-device LAN/WSS proof; scheduler/preflight counts need an exact command/log before citation)
