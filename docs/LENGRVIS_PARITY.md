...
As of the latest 2026-06-09 development handoff, targeted evidence in this dirty workspace records desktop/mobile typecheck passing, `npm --prefix desktop run smoke:first-launch`, mobile token/task-companion/remote-input smokes, core backend combo `183 passed`, task explain/tasks replay/mobile pairing combo `107 passed`, `backend/tests/test_mobile_pairing.py` `88 passed`, task explain/tasks replay/remote desktop combo `45 passed`, `backend/tests/test_remote_desktop.py` `28 passed`, Ollama backend tests `52 passed`, and `git diff --check` exit 0 with LF-to-CRLF working-copy warnings only. The previously tracked P1 review findings plus this round's public task text bare-filename/hidden-prompt leakage, realtime raw malformed-message sampling, and mobile QR transport metadata bypass findings are closed. This is useful productization evidence, but it is not clean-machine RC sign-off, real phone/WSS proof, true local model install/start/pull proof, completed task-result sign-off, result-quality sign-off, or external diagnostics human review.
...
| 打包、发布与 QA gate | `qa:gate`、`release:check`、`release:quick`、runnable backend smoke、portable launcher/backend diagnostics smoke、portable renderer DOM read-only entry smoke、portable natural-language `/api/runs` submission/task-evidence smoke、dependency lock verification、release safety gate 已整理；当前 dirty workspace 另有 core backend combo `183 passed`、desktop/mobile typecheck、first-launch smoke、mobile behavior smokes、mobile_pairing `88 passed`，以及 prior + latest P1 review findings closed。证据：`package.json`、`docs/qa/release-gate.md`、`docs/qa/e2e-acceptance-matrix.md`、`scripts/portable_first_screen_smoke.ps1` | Windows artifact structural + backend runnable smoke + portable 窗口/health/local-only diagnostics + packaged renderer read-only click path + packaged natural-language command dock submission/task evidence 可演示；special offline Ollama release 有显式 `-RequireBundledOllama` 验证入口 | 真实机器候选启动、packaged GUI 自然语言任务进度/结果、completed task-result sign-off、result-quality sign-off、macOS DMG、Android 分发证据仍需人工或平台验收；当前 packaged natural-language smoke 只记录 submission + backend read-only/system diagnostics task evidence，不证明完整任务结果、clean-machine 或 RC 签收 | iOS 分发、自动更新、完整 crash/diagnostic pipeline |
...
- Focused mobile/remote evidence: `python -m pytest backend/tests/test_mobile_pairing.py backend/tests/test_lan_api_guard.py backend/tests/test_remote_desktop.py -q` (current targeted file reruns: mobile_pairing `88 passed`, remote desktop `28 passed`)
