import { Check, FileKey2, Loader2, RefreshCw, ShieldAlert } from "lucide-react";
...
function quotaSummary(quota: CommerceQuotaStatus | null): string {
  ...
  if (!quota.enforced) return "已计量，暂未强制";
  const used = quota.usage?.totalTokens ?? 0;
  const limit = quota.limits.totalTokens;
  return limit === null ? "不设产品内上限" : `${formatNumber(used)} / ${formatNumber(limit)} tokens`;
}
