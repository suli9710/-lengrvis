...
(async () => {
  try {
    ...
    assert.equal(
      fs.readFileSync(path.join(process.env.LENGRVIS_DATA_DIR, "native_confirmation_public.key"), "utf8").trim(),
      nativeConfirmationPublicKey
    );
    assert.equal(fs.existsSync(path.join(process.env.LENGRVIS_DATA_DIR, "native_confirmation.secret")), false);
