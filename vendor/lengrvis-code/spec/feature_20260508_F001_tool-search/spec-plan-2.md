...
- [x] 在 `src/constants/prompts.ts` 中添加 ToolSearch + ExecuteTool 引导指令到系统提示词
  ...
  - 在文件顶部 import 区域新增:
    ...
    ```
  - 注意: `TOOL_SEARCH_TOOL_NAME` 已通过 `src/constants/tools.ts` 的 import 链路导入（L25 `import { TOOL_SEARCH_TOOL_NAME } from '@lengrvis-code-best/builtin-tools/tools/ToolSearchTool/prompt.js'`），无需重复导入。但需在 `prompts.ts` 中新增 `EXECUTE_TOOL_NAME` 的 import（当前文件中无此 import，经 grep 确认）。
...
`
  *   (unset)               tst (default: always defer non-core tools)
  ...
  return 'tst' // default: always defer non-core tools
    ```
  - 注意: `shouldDefer` 在此文件中仅出现在注释中（L4, L155, L170, L197），无任何运行时引用。`isDeferredTool` 函数从 `@lengrvis-code-best/builtin-tools/tools/ToolSearchTool/prompt.js` 导入（L24），Task 1 已将其重构为白名单制，此处无需修改函数调用。
...
- [x] 新建 `src/components/ToolSearchHint.tsx` — Ink 组件，渲染工具推荐提示条
  - 位置: 新建文件，参照 `src/components/LengrvisCodeHint/PluginHintMenu.tsx` 的结构模式
  - 组件签名:
    ```typescript
...
- [x] 新建 `src/hooks/useToolSearchHint.ts` — 自定义 Hook，管理工具推荐状态与生命周期
  - 位置: 新建文件，参照 `src/hooks/useLengrvisCodeHintRecommendation.tsx` 的状态管理模式
  - Hook 签名:
    ```typescript
  ...
  - 原因: 将状态管理与 UI 渲染解耦，遵循现有 hook 模式（useLengrvisCodeHintRecommendation）
...
- [x] 修改 `src/screens/REPL.tsx` — 集成 ToolSearchHint 组件到对话框优先级系统
  ...
  - 位置: 文件顶部 import 区域（~L448，`PluginHintMenu` import 附近），新增 import:
    ...
    ```
  - 位置: hook 调用区域（~L1038，`useLengrvisCodeHintRecommendation` 调用之后），新增:
    ```typescript
