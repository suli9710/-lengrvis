...
- [x] 移除背景任务说明的 `!forkEnabled` 条件
  ...
  - 将条件从 `!isEnvTruthy(process.env.LENGRVIS_CODE_DISABLE_BACKGROUND_TASKS) && !isInProcessTeammate() && !forkEnabled` 改为 `!isEnvTruthy(process.env.LENGRVIS_CODE_DISABLE_BACKGROUND_TASKS) && !isInProcessTeammate()`
...
- [x] 为 prompt.ts 的 fork 相关文本变更编写单元测试
  ...
  - Mock 列表: `isForkSubagentEnabled`（返回 true/false）、`getFeatureValue_CACHED_MAY_BE_STALE`（返回 false）、`shouldInjectAgentListInMessages`（返回 false）、`isInProcessTeammate`（返回 false）、`isTeammate`（返回 false）、`getSubscriptionType`（返回 'pro'）、`hasEmbeddedSearchTools`（返回 false）、环境变量 `LENGRVIS_CODE_DISABLE_BACKGROUND_TASKS` 未定义
