...
This file provides guidance to Lengrvis Code (claude.ai/code) and other AI coding agents when working with code in this repository.

## Project Overview

This is a **reverse-engineered / decompiled** version of Anthropic's official Lengrvis Code CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced — **`bunx tsc --noEmit` must pass with zero errors**.
...
- **`src/tools.ts`** — Tool registry. Assembles the tool list; tools are imported from `@lengrvis-code-best/builtin-tools` package. Some tools are conditionally loaded via `feature()` flags or `process.env.USER_TYPE`.
- **`packages/builtin-tools/src/tools/`** — 59 个子目录（含 shared/testing 等工具目录），通过 `@lengrvis-code-best/builtin-tools` 包导出。主要分类：
...
| `packages/builtin-tools/` | 内置工具集（60 个 tool 实现，通过 `@lengrvis-code-best/builtin-tools` 导出） |
| `packages/agent-tools/` | Agent 工具集 |
| `packages/acp-link/` | ACP 代理服务器（WebSocket → ACP agent 桥接） |
| `packages/cc-knowledge/` | Lengrvis Code 知识库（非 workspace 包） |
...
- **`src/services/acp/`** — ACP agent 实现，包含 `agent.ts`（AcpAgent 类）、`bridge.ts`（Lengrvis Code ↔ ACP 桥接）、`permissions.ts`（权限处理）、`entry.ts`（入口）。
...
通过 `LENGRVIS_CODE_USE_OPENAI=1` 启用，支持 Ollama/DeepSeek/vLLM 等任意 OpenAI Chat Completions 协议端点。含 DeepSeek thinking mode 支持。

- **`src/services/api/openai/`** — client、消息/工具转换、流适配、模型映射
- 关键环境变量：`LENGRVIS_CODE_USE_OPENAI`、`OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`

#### Gemini 兼容层

通过 `LENGRVIS_CODE_USE_GEMINI=1` 启用。独立环境变量体系。
...
通过 `LENGRVIS_CODE_USE_GROK=1` 启用。自定义模型映射支持 xAI Grok API。
