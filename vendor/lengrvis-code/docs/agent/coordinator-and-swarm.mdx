...
description: "从源码角度解析 Lengrvis Code 多 Agent 协作：Coordinator Mode 的 System Prompt 设计、Worker 生命周期、Task 通信协议和 Swarm 蜂群的任务分配机制。"
...
|------|-----------------|--------------|
| **门控** | `feature('COORDINATOR_MODE')` + `LENGRVIS_CODE_COORDINATOR_MODE=1` | `LENGRVIS_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 环境变量 |
...
所有 Teammate 自动获得相同的 taskListId
  ↓
Teammate 启动时：
  1. LENGRVIS_CODE_TASK_LIST_ID 环境变量（显式覆盖）
  2. Teammate 上下文的 teamName（共享 Lead 的任务列表）
  3. LENGRVIS_CODE_TEAM_NAME 环境变量
  4. Lead 设置的 teamName
  5. getSessionId()（兜底）
```
...
| 组件 | 角色 |
|------|------|
| **Team Lead** | 创建团队、分配任务、综合结果的主 Lengrvis Code 会话 |
| **Teammate** | 独立的 Lengrvis Code 实例，各自拥有独立的上下文窗口 |
