...
description: "从源码角度揭示 Lengrvis Code token 预算管理：200K 上下文窗口的动态计算、截断机制、缓存优化和自动压缩的完整链路。"
...
Lengrvis Code 的默认上下文窗口为 200K tokens（`MODEL_CONTEXT_WINDOW_DEFAULT = 200_000`），但实际可用于对话的空间远小于此：
...
1. `LENGRVIS_CODE_MAX_CONTEXT_TOKENS` 环境变量覆盖
