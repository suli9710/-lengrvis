...
description: "深入解析 Lengrvis Code 的 System Prompt 动态组装过程：缓存策略、分界标记、Section 注册表、CLAUDE.md 多级合并，以及如何将零散上下文拼装为 API 可消费的缓存友好结构。"
keywords: ["System Prompt", "系统提示词", "动态组装", "CLAUDE.md", "Prompt Cache", "缓存策略"]
---

## 从数组到 API 调用：System Prompt 的完整链路

System Prompt 在 Lengrvis Code 中不是一段写死的文本，而是一个 **`string[]` 数组**（品牌类型 `SystemPrompt`，定义于 `src/utils/systemPromptType.ts:8`），经过组装、分块、缓存标记后发送给 API。
...
### `LENGRVIS_CODE_SIMPLE` 快速路径

当环境变量 `LENGRVIS_CODE_SIMPLE` 为真时，整个 System Prompt 缩减为一行：
...
Lengrvis Code 支持多种 API 提供商，分为两大类：

| 类别 | Provider | 环境变量 | 说明 |
|------|----------|---------|------|
| **1P (First Party)** | `firstParty` | 默认 | Anthropic 官方 API 直连 |
| **3P (Third Party)** | `bedrock` | `LENGRVIS_CODE_USE_BEDROCK=1` | AWS Bedrock 托管服务 |
| **3P** | `vertex` | `LENGRVIS_CODE_USE_VERTEX=1` | Google Vertex AI |
| **3P** | `openai` | `LENGRVIS_CODE_USE_OPENAI=1` | OpenAI 兼容层（Ollama/DeepSeek/vLLM） |
| **3P** | `gemini` | `LENGRVIS_CODE_USE_GEMINI=1` | Google Gemini API |
| **3P** | `grok` | `LENGRVIS_CODE_USE_GROK=1` | xAI Grok |
...
- **1P 用户禁用实验性功能**：设置 `LENGRVIS_CODE_DISABLE_EXPERIMENTAL_BETAS=1`，Boundary 不插入
...
### `LENGRVIS_CODE_SIMPLE` 模式

当设置了 `LENGRVIS_CODE_SIMPLE` 环境变量时，整个系统提示词会大幅缩减：

```typescript
return [`You are Lengrvis Code, Anthropic's official CLI for Claude.\n\nCWD: ${getCwd()}\nDate: ${getSessionStartDate()}`]
```
...
- **CLAUDE.md 禁用条件**：`LENGRVIS_CODE_DISABLE_CLAUDE_MDS` 环境变量，或 `--bare` 模式（除非通过 `--add-dir` 显式指定目录）
...
- **`cc_version`**：Lengrvis Code 版本 + 指纹
- **`cc_entrypoint`**：入口点标识（REPL / SDK / pipe 等）
- **`cch=00000`**（NATIVE_CLIENT_ATTESTATION 启用时）：Bun 原生 HTTP 层在发送前将零替换为计算出的哈希值，服务器验证此 token 确认请求来自真实 Lengrvis Code 客户端

Header 始终 `cacheScope: null`——它因版本和指纹不同而变化，不适合缓存。

## CLAUDE.md：项目级知识注入

这是 Lengrvis Code 最巧妙的设计之一。在项目根目录放一个 `CLAUDE.md` 文件，就能让 AI "理解" 你的项目：
...
这是 Lengrvis Code 在 token 成本优化上的核心设计——一次典型的 System Prompt 约 20K+ tokens，通过缓存分块可以节省 30-50% 的输入 token 费用。

## 兼容层：OpenAI 与 Gemini

Lengrvis Code 提供了 OpenAI 和 Gemini 协议的兼容层，允许使用非 Anthropic 端点。

### OpenAI 兼容层

通过 `LENGRVIS_CODE_USE_OPENAI=1` 启用，支持任意 OpenAI Chat Completions 协议端点（Ollama、DeepSeek、vLLM 等）。
...
关键环境变量：
- `LENGRVIS_CODE_USE_OPENAI=1` — 启用 OpenAI provider
...
通过 `LENGRVIS_CODE_USE_GEMINI=1` 启用，支持 Google Gemini API。
...
关键环境变量：
- `LENGRVIS_CODE_USE_GEMINI=1` — 启用 Gemini provider
