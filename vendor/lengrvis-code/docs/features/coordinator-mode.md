...
> Feature Flag: `FEATURE_COORDINATOR_MODE=1` + 环境变量 `LENGRVIS_CODE_COORDINATOR_MODE=1`
...
```bash
FEATURE_COORDINATOR_MODE=1 LENGRVIS_CODE_COORDINATOR_MODE=1 bun run dev
```

需要同时设置 feature flag 和环境变量。`LENGRVIS_CODE_COORDINATOR_MODE` 可在会话恢复时自动切换（`matchSessionMode`）。
...
export function isCoordinatorMode(): boolean {
  return feature('COORDINATOR_MODE') &&
    isEnvTruthy(process.env.LENGRVIS_CODE_COORDINATOR_MODE)
}
```
...
- **Simple 模式**（`LENGRVIS_CODE_SIMPLE=1`）：仅 Bash、Read、Edit
...
LENGRVIS_CODE_COORDINATOR_MODE=1 bun run dev

# Simple 模式（worker 只有 Bash/Read/Edit）
FEATURE_COORDINATOR_MODE=1 LENGRVIS_CODE_COORDINATOR_MODE=1 \
LENGRVIS_CODE_SIMPLE=1 bun run dev
```
