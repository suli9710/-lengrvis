...
本指南说明如何将 Remote Control Server (RCS) 部署到私有环境，并通过 Lengrvis Code CLI 连接使用。

## 架构概览

```
┌──────────────────┐                    ┌──────────────────────┐
│  Lengrvis Code CLI  │ ◄── HTTP/SSE/WS ─►│  Remote Control      │
...
- 接收 Lengrvis Code CLI 的环境注册和工作轮询
...
- 一台可被 Lengrvis Code CLI 和 Web 浏览器同时访问的服务器（物理机、VM、容器均可）
- [Docker](https://www.docker.com/)
- 启用 `BRIDGE_MODE` feature flag 的 Lengrvis Code 构建
...
| `LENGRVIS_CODE_REMOTE` | 否 | 设为 `1` 时标记为远程执行模式 |

## Lengrvis Code 客户端连接

### 1. 设置环境变量

在运行 Lengrvis Code 的机器上设置：
...
ACP session 在 Web UI 中显示品牌色标签，与普通 Lengrvis Code session 区分。
...
└──────────────────────────────────────────────────────────┘
 ...
 1. Lengrvis Code CLI 启动，设置环境变量指向自托管 RCS
