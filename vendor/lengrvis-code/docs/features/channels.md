...
Channel 是一个 MCP 服务器，它将外部事件推送到你运行中的 Lengrvis Code 会话中，以便 Claude 可以在你不在终端时做出反应。详细使用说明请参考以下文档：

- **官方文档**：[使用 channels 将事件推送到运行中的会话](https://code.claude.com/docs/zh-CN/channels)
- **飞书插件**：[lengrvis-code-feishu-channel](https://github.com/whobot-ai/lengrvis-code-feishu-channel) — 社区首个飞书 Channel 插件，支持双向消息、配对认证、群组聊天、文件附件
...
```bash
# 启用频道监听（plugin 格式）
ccb --channels plugin:feishu@lengrvis-code-feishu-channel
...
# 同时启用多个频道
ccb --channels plugin:feishu@lengrvis-code-feishu-channel --channels server:discord-bot
...
| **飞书 (Feishu/Lark)** | 双向消息、群组聊天、文件附件 | `/plugin install feishu@lengrvis-code-feishu-channel` |
...
确认后，该微信用户后续消息才会进入 Lengrvis Code 会话。
...
- [飞书 Channel 插件](https://github.com/whobot-ai/lengrvis-code-feishu-channel) — 安装配置教程、MCP 工具、Skill 命令参考
