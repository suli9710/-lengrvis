...
```
GET  /api/lengrvis_code/team_memory?repo={owner/repo}             → 完整数据 + entryChecksums
GET  /api/lengrvis_code/team_memory?repo={owner/repo}&view=hashes → 仅 checksums（冲突解决用）
PUT  /api/lengrvis_code/team_memory?repo={owner/repo}             → 上传 entries（upsert 语义）
```
...
| Team Memory API | `/api/lengrvis_code/team_memory` 端点 |
