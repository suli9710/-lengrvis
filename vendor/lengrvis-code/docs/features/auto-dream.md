...
Auto Dream 是 Lengrvis Code 的后台记忆整合机制。它在会话间自动审查、组织和修剪持久化记忆文件，确保未来会话能快速获得准确的上下文。
...
其中 `memoryBase` = `LENGRVIS_CODE_REMOTE_MEMORY_DIR` 或 `~/.claude`。
...
| `LENGRVIS_CODE_DISABLE_AUTO_MEMORY` | 环境变量 | `1`/`true` 关闭所有记忆功能 |
...
开发者连续多天使用 Lengrvis Code 处理不同任务。Auto Dream 在积累 5+ 个会话且距上次整理 24 小时后自动触发，整合分散在多次会话中的用户偏好和项目决策。
