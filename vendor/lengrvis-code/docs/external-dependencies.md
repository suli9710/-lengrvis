...
| 2 | AWS Bedrock | `bedrock-runtime.*.amazonaws.com` | HTTPS | 需 `LENGRVIS_CODE_USE_BEDROCK=1` |
| 3 | Google Vertex AI | `{region}-aiplatform.googleapis.com` | HTTPS | 需 `LENGRVIS_CODE_USE_VERTEX=1` |
| 4 | Azure Foundry | `{resource}.services.ai.azure.com` | HTTPS | 需 `LENGRVIS_CODE_USE_FOUNDRY=1` |
...
| 11 | BigQuery Metrics | `api.anthropic.com/api/lengrvis_code/metrics` | HTTPS | 默认启用 |
...
- **端点**:
  ...
  - `https://claude.ai/oauth/lengrvis-code-client-metadata` — MCP 客户端元数据
...
- **激活**: `LENGRVIS_CODE_ENABLE_TELEMETRY=1` 或 `OTEL_*` 环境变量
...
- **端点**: `https://api.anthropic.com/api/lengrvis_code/metrics`
...
- **端点**: `https://storage.googleapis.com/lengrvis-code-dist-86c565f3-f756-42ad-8dfa-d59b1c096819/lengrvis-code-releases`
- **文件**: `src/utils/autoUpdater.ts`

### 16. GitHub Raw Content

- **端点**: `https://raw.githubusercontent.com/anthropics/lengrvis-code/refs/heads/main/CHANGELOG.md`
...
- **激活**: `LENGRVIS_CODE_REMOTE=1` + `CCR_UPSTREAM_PROXY_ENABLED=1`
...
| `/api/lengrvis_code/metrics` | BigQuery 指标导出 | `src/utils/telemetry/bigqueryExporter.ts` |
...
| `/api/lengrvis_code/settings` | 设置同步 | `src/services/settingsSync/index.ts` |
| `/api/lengrvis_code/managed_settings` | 企业托管设置 (1h 轮询) | `src/services/remoteManagedSettings/index.ts` |
| `/api/lengrvis_code/team_memory?repo={}` | 团队记忆同步 | `src/services/teamMemorySync/index.ts` |
| `/api/auth/trusted_devices` | 可信设备注册 | `src/bridge/trustedDevice.ts` |
| `/api/organizations/{id}/lengrvis_code/buddy_react` | Companion 反应 | `src/buddy/companionReact.ts` |
