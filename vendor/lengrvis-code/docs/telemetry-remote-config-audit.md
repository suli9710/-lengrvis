...
- **Proto schema**: 事件序列化为 `LengrvisCodeInternalEvent` / `GrowthbookExperimentEvent` protobuf 格式
...
- **端点**: `{BASE_API_URL}/api/lengrvis_code/settings`
...
- **端点**: `{BASE_API_URL}/api/lengrvis_code/user_settings`
...
- **触发**: `LENGRVIS_CODE_ENABLE_TELEMETRY=1` 环境变量
...
- **端点**: `https://api.anthropic.com/api/lengrvis_code/organizations/metrics_enabled`
...
- **详细模式**: `LENGRVIS_CODE_PROFILE_STARTUP=1` 输出完整性能报告到文件
...
LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1

# 3P 提供商自动禁用
LENGRVIS_CODE_USE_BEDROCK=1  # 或 VERTEX/FOUNDRY
```
...
用户操作 → logEvent()
   ...
   (us5.datadoghq.com)       ↓
                    ...
                    api.anthropic.com/api/event_logging/batch
                             ↓
                    BigQuery (LengrvisCodeInternalEvent proto)
```
