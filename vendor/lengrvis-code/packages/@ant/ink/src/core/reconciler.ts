...
export function isDebugRepaintsEnabled(): boolean {
  if (debugRepaints === undefined) {
    debugRepaints = process.env.LENGRVIS_CODE_DEBUG_REPAINTS === '1'
  }
...
// Set LENGRVIS_CODE_COMMIT_LOG=1 to enable debug logging to stderr.
const COMMIT_LOG = process.env.LENGRVIS_CODE_COMMIT_LOG
let _commits = 0
let _lastLog = 0
let _lastCommitAt = 0
let _maxGapMs = 0
