...
Remote Control Server 是 Lengrvis Code 的远程控制后端，允许你通过浏览器 Web UI 远程监控和操作 Lengrvis Code 会话。

## 功能

- **会话管理** — 创建、监控、归档 Lengrvis Code 会话
- **实时消息流** — WebSocket / SSE 双向传输，实时查看对话和工具调用
- **权限审批** — 在 Web UI 中审批 Lengrvis Code 的工具权限请求
...
docker run -d \
  ...
  -v rcs-data:/app/data \
  ghcr.io/lengrvis-code-best/remote-control-server:latest
```
...
| `LENGRVIS_CODE_REMOTE` | 设为 `1` 时标记为远程执行模式 |
