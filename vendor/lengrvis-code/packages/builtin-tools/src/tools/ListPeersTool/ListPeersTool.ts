...
export const ListPeersTool = buildTool({
  ...
  async description() {
    return 'Discover other Lengrvis Code sessions for cross-session messaging'
  },
  async prompt() {
    return `List active Lengrvis Code sessions that can receive messages via SendMessage.
