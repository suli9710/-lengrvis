...
export const TeamCreateTool: Tool<InputSchema, Output> = buildTool({
  ...
  async call(input, context) {
    if (!isAgentSwarmsEnabled()) {
      throw new Error(
        'Agent Teams 功能未启用。请确保未设置 LENGRVIS_CODE_EXPERIMENTAL_AGENT_TEAMS_DISABLED 环境变量。',
      )
    }
    ...
    // Note: We intentionally don't set LENGRVIS_CODE_AGENT_ID for the team lead because:
