 ...
 *
 * Priority order:
 * 1. LENGRVIS_CODE_URL_EVENT — set by the OS URL scheme handler on activation
 * 2. LENGRVIS_CODE_DEEP_LINK_URL — set by the desktop app launcher
 * 3. LENGRVIS_CODE_URL — legacy / manual override
 * 4. CLI arguments — e.g. `claude claude://...`
 */
function findUrlEvent(): string | null {
  for (const key of [
    'LENGRVIS_CODE_URL_EVENT',
    'LENGRVIS_CODE_DEEP_LINK_URL',
    'LENGRVIS_CODE_URL',
  ]) {
