...
const originalEnv = {
  LENGRVIS_CODE_URL_EVENT: process.env.LENGRVIS_CODE_URL_EVENT,
  LENGRVIS_CODE_DEEP_LINK_URL: process.env.LENGRVIS_CODE_DEEP_LINK_URL,
  LENGRVIS_CODE_URL: process.env.LENGRVIS_CODE_URL,
}
...
describe('waitForUrlEvent', () => {
  ...
  test('rejects URLs exceeding the maximum length', async () => {
    process.env.LENGRVIS_CODE_URL_EVENT = `claude-cli://${'x'.repeat(2048)}`
