  ...
  async *submitMessage(
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.LENGRVIS_CODE_EAGER_FLUSH) ||
          isEnvTruthy(process.env.LENGRVIS_CODE_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via LENGRVIS_CODE_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or LENGRVIS_CODE_PLUGIN_SEED_DIR before this runs;
