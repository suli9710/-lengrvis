...
export const init = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    // LENGRVIS_CODE_REMOTE + GrowthBook; fail-open on any error. Lazy import so
...
async function doInitializeTelemetry(): Promise<void> {
  ...
  if (!isEnvTruthy(process.env.LENGRVIS_CODE_ENABLE_TELEMETRY)) {
    telemetryInitialized = true
    logForDebugging(
      '[3P telemetry] Skipped — LENGRVIS_CODE_ENABLE_TELEMETRY not set',
    )
    return
  }
