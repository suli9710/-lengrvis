...
if (typeof globalThis.MACRO === 'undefined') {
  (globalThis as any).MACRO = {
    VERSION: process.env.LENGRVIS_CODE_VERSION || '2.1.888',
...
if (isEnvTruthy(process.env.LENGRVIS_CODE_FORCE_INTERACTIVE)) {
...
if (process.env.LENGRVIS_CODE_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.LENGRVIS_CODE_ABLATION_BASELINE) {
  for (const k of [
    'LENGRVIS_CODE_SIMPLE',
    'LENGRVIS_CODE_DISABLE_THINKING',
    'DISABLE_INTERLEAVED_THINKING',
    'DISABLE_COMPACT',
    'DISABLE_AUTO_COMPACT',
    'LENGRVIS_CODE_DISABLE_AUTO_MEMORY',
    'LENGRVIS_CODE_DISABLE_BACKGROUND_TASKS',
  ]) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Lengrvis Code)`);
    return;
  }
  ...
  if (args[0] === 'weixin') {
    ...
    const { handleWeixinCli } = await import('@lengrvis-code-best/weixin');
    const { enableConfigs } = await import('../utils/config.js');
