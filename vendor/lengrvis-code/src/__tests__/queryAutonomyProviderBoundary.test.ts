...
describe('query autonomy/provider boundary', () => {
  test('provider api-error messages fail a consumed autonomy run instead of advancing the flow', async () => {
    const previousDisableAttachments =
      process.env.LENGRVIS_CODE_DISABLE_ATTACHMENTS
    process.env.LENGRVIS_CODE_DISABLE_ATTACHMENTS = '1'
    try {
      ...
      if (previousDisableAttachments === undefined) {
        delete process.env.LENGRVIS_CODE_DISABLE_ATTACHMENTS
      } else {
        process.env.LENGRVIS_CODE_DISABLE_ATTACHMENTS = previousDisableAttachments
      }
    }
  })
