...
export function getLocalISODate(): string {
  ...
  if (process.env.LENGRVIS_CODE_OVERRIDE_DATE) {
    return process.env.LENGRVIS_CODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.LENGRVIS_CODE_OVERRIDE_DATE
    ? new Date(process.env.LENGRVIS_CODE_OVERRIDE_DATE)
    : new Date()
