...
export function MCPRemoteServerMenu({
  ...
  const handleClaudeAIAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        onComplete?.(
          'Authentication successful, but server still requires authentication. You may need to manually restart Lengrvis Code.',
        );
      } else {
        onComplete?.(
          'Authentication successful, but server reconnection failed. You may need to manually restart Lengrvis Code for the changes to take effect.',
        );
      }
    } catch (err) {
  ...
  const handleClaudeAIAuth = React.useCallback(async () => {
    ...
    if (orgUuid && server.config.type === 'claudeai-proxy' && server.config.id) {
      ...
      const productSurface = encodeURIComponent(process.env.LENGRVIS_CODE_ENTRYPOINT || 'cli');
