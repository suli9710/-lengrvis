...
export function ResumeTask({ onSelect, onCancel, isEmbedded = false }: Props): React.ReactNode {
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading Lengrvis Code sessions…</Text>
        </Box>
        <Text dimColor>{retrying ? 'Retrying…' : 'Fetching your Lengrvis Code sessions…'}</Text>
      </Box>
    );
  }
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading Lengrvis Code sessions
        </Text>
  ...
  if (sessions.length === 0) {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold>
          No Lengrvis Code sessions found
    ...
    case 'other':
      return (
        <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, Lengrvis Code encountered an error</Text>
        </Box>
      );
  }
}
