...
export function useUnseenDivider(messageCount: number): {
  ...
  const onScrollAway = useCallback((handle: ScrollBoxHandle) => {
    ...
    //   at max (Sarah Deaton, #lengrvis-code-feedback 2026-03-15)
 ...
 *
 * Fullscreen mode defaults on for ants (LENGRVIS_CODE_NO_FLICKER=0 to opt out)
 * and off for external users (LENGRVIS_CODE_NO_FLICKER=1 to opt in).
...
export function FullscreenLayout({
  ...
  useLayoutEffect(() => {
    ...
    ink.onHyperlinkClick = url => {
      // Most OSC 8 links emitted by Lengrvis Code are file:// URLs from
