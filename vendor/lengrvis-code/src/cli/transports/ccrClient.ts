 ...
 * - Epoch management: reads worker_epoch from LENGRVIS_CODE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (LENGRVIS_CODE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * LENGRVIS_CODE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    LENGRVIS_CODE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.LENGRVIS_CODE_WORKER_EPOCH
