...
import { fetchAndStoreLengrvisCodeFirstTokenDate } from '../../services/api/firstTokenDate.js'
import {
...
export async function installOAuthTokens(tokens: OAuthTokens): Promise<void> {
  ...
  if (shouldUseClaudeAIAuth(tokens.scopes)) {
    await fetchAndStoreLengrvisCodeFirstTokenDate().catch(err =>
      logForDebugging(String(err), { level: 'error' }),
    )
  } else {
...
export async function authLogin({
  ...
  const envRefreshToken = process.env.LENGRVIS_CODE_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.LENGRVIS_CODE_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'LENGRVIS_CODE_OAUTH_SCOPES is required when using LENGRVIS_CODE_OAUTH_REFRESH_TOKEN.\n' +
          'Set it to the space-separated scopes the refresh token was issued with\n' +
          '(e.g. "user:inference" or "user:profile user:inference user:sessions:lengrvis_code user:mcp_servers").\n',
      )
      process.exit(1)
    }
