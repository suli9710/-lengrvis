/**
 * Surfaces plugin-install prompts driven by `<lengrvis-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/lengrvis-code-hints.md.
 *
...
} from '../utils/lengrvisCodeHints.js';
...
type UseLengrvisCodeHintRecommendationResult = {
  recommendation: PluginHintRecommendation | null;
  handleResponse: (response: 'yes' | 'no' | 'disable') => void;
};

export function useLengrvisCodeHintRecommendation(): UseLengrvisCodeHintRecommendationResult {
  ...
  React.useEffect(() => {
    ...
    tryResolve(async () => {
      ...
      if (resolved) {
        logForDebugging(
          `[useLengrvisCodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`,
        );
        markShownThisSession();
      }
