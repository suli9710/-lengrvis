...
function getChatGPTResponsesReasoningEffort(
  ...
  const envOverride = process.env.LENGRVIS_CODE_EFFORT_LEVEL?.toLowerCase()
    ...
    outputs.push(
      createAssistantAPIErrorMessage({
        content:
          ...
          `Set OPENAI_MAX_TOKENS or LENGRVIS_CODE_MAX_OUTPUT_TOKENS to override.`,
...
> {
  try {
    ...
    //     3. LENGRVIS_CODE_MAX_OUTPUT_TOKENS env var (generic override)
