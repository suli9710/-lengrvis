...
// OAuth credentials as Lengrvis Code.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Lengrvis Code — available when the
...
export async function connectVoiceStream(
  ...
  // non-browser clients (anthropics/lengrvis-code#34094). Same private-api
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/lengrvis-code#40510).
