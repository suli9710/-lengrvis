...
mock.module('src/services/auth/hostGuard.ts', () => ({
  ...
  assertNoAnthropicEnvForOpenAI(): void {
    const hasOpenAIMode =
      process.env['LENGRVIS_CODE_USE_OPENAI'] === '1' ||
...
describe('assertNoAnthropicEnvForOpenAI', () => {
  ...
  const origOpenAIMode = process.env['LENGRVIS_CODE_USE_OPENAI']
  ...
  afterEach(() => {
    ...
    if (origOpenAIMode === undefined) {
      ...
      process.env['LENGRVIS_CODE_USE_OPENAI'] = origOpenAIMode
    }
  })
  ...
  test('does not throw (only warns) when both ANTHROPIC_API_KEY and LENGRVIS_CODE_USE_OPENAI=1 are set', () => {
    process.env['ANTHROPIC_API_KEY'] = 'sk-ant-api03-test'
    process.env['LENGRVIS_CODE_USE_OPENAI'] = '1'
    // Must NOT throw
    expect(() => assertNoAnthropicEnvForOpenAI()).not.toThrow()
