 ...
 *   GET  /api/lengrvis_code/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/lengrvis_code/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/lengrvis_code/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
import { getLengrvisCodeUserAgent } from '../../utils/userAgent.js'
...
// GB-tunable per-org (lengrvis_code_team_memory_limits), so any compile-time
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/lengrvis_code/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
  ...
  if (oauthTokens?.accessToken) {
    return {
      headers: {
        ...
        'User-Agent': getLengrvisCodeUserAgent(),
      },
    }
  }
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // lengrvis_code_team_memory_limits; we have no way to know it in advance.
