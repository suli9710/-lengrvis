  ...
  if (!process.env[found.apiKeyEnv]) {
    warnings.push(
      `${found.apiKeyEnv} is not set in the current environment. ` +
        `Set it before starting Lengrvis Code: export ${found.apiKeyEnv}=<your-api-key>`,
    )
  }
 ...
 *
 * Example output:
 *   export LENGRVIS_CODE_USE_OPENAI=1
...
export function buildShellExportBlock(result: SwitchProviderResult): string {
  ...
  const lines: string[] = [
    `export LENGRVIS_CODE_USE_OPENAI=${env['LENGRVIS_CODE_USE_OPENAI'] ?? '1'}`,
