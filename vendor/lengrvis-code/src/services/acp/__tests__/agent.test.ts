...
const originalAcpAllowBypass =
  process.env.LENGRVIS_CODE_ACP_ALLOW_BYPASS_PERMISSIONS
...
afterAll(() => {
  ...
  restoreEnv('LENGRVIS_CODE_ACP_ALLOW_BYPASS_PERMISSIONS', originalAcpAllowBypass)
})
...
describe('AcpAgent', () => {
  beforeEach(() => {
    ...
    delete process.env.LENGRVIS_CODE_ACP_ALLOW_BYPASS_PERMISSIONS
  ...
  describe('initialize', () => {
    test('returns protocol version and agent info', async () => {
      ...
      expect(res.agentInfo?.name).toBe('lengrvis-code')
  ...
  describe('newSession', () => {
    ...
    test('honors _meta.permissionMode bypass with a local ACP bypass gate', async () => {
      process.env.LENGRVIS_CODE_ACP_ALLOW_BYPASS_PERMISSIONS = '1'
      const agent = new AcpAgent(makeConn())
      const res = await agent.newSession({
        cwd: '/tmp',
        _meta: { permissionMode: 'bypassPermissions' },
