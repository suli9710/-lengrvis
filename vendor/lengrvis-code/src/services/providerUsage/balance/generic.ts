 ...
 *
 * Environment:
 *   LENGRVIS_CODE_BALANCE_URL        — GET endpoint returning JSON (required)
 *   LENGRVIS_CODE_BALANCE_KEY        — optional Bearer token (falls back to OPENAI_API_KEY / ANTHROPIC_API_KEY)
 *   LENGRVIS_CODE_BALANCE_JSON_PATH  — dot path into the JSON for the remaining number (default: "balance")
 *                                    array indices allowed, e.g. "data.0.credit"
 *   LENGRVIS_CODE_BALANCE_CURRENCY   — display currency label (default: "USD")
 *
...
export const genericBalanceProvider: BalanceProvider = {
  ...
  isEnabled(): boolean {
    return Boolean(process.env.LENGRVIS_CODE_BALANCE_URL)
  },

  async fetchBalance(signal?: AbortSignal): Promise<ProviderBalance | null> {
    const rawUrl = process.env.LENGRVIS_CODE_BALANCE_URL
    ...
    // WARNING: fallback keys are sent to LENGRVIS_CODE_BALANCE_URL as Bearer token.
    // If that URL is untrusted, your provider key leaks. Prefer LENGRVIS_CODE_BALANCE_KEY.
    const key =
      process.env.LENGRVIS_CODE_BALANCE_KEY ||
      process.env.OPENAI_API_KEY ||
      process.env.ANTHROPIC_API_KEY ||
      ''
    const path = process.env.LENGRVIS_CODE_BALANCE_JSON_PATH || 'balance'
    const currency = process.env.LENGRVIS_CODE_BALANCE_CURRENCY || 'USD'
