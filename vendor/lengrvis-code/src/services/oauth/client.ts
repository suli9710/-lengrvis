...
export async function refreshOAuthToken(
  ...
  try {
    ...
    // LENGRVIS_CODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.LENGRVIS_CODE_ACCOUNT_UUID
  const envUserEmail = process.env.LENGRVIS_CODE_USER_EMAIL
  const envOrganizationUuid = process.env.LENGRVIS_CODE_ORGANIZATION_UUID
