 ...
 * in getLengrvisCodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getLengrvisCodeMcpConfigs().
...
/**
 * Get Lengrvis Code MCP configurations (excludes claude.ai servers from the
 ...
 * @returns Lengrvis Code server configurations with appropriate scopes
 */
export async function getLengrvisCodeMcpConfigs(
 ...
 * This may be slow due to network calls - use getLengrvisCodeMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  if (doesEnterpriseMcpConfigExist()) {
    return getLengrvisCodeMcpConfigs()
  }

  // Kick off the claude.ai fetch before getLengrvisCodeMcpConfigs so it overlaps
  ...
  const { servers: lengrvisCodeServers, errors } = await getLengrvisCodeMcpConfigs(
    {},
    claudeaiPromise,
  )
  ...
  const servers = Object.assign({}, dedupedClaudeAi, lengrvisCodeServers)
