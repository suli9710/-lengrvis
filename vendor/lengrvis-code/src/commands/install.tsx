...
function Install({ onDone, force, target }: InstallProps): React.ReactNode {
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'Lengrvis Code installation completed successfully', {
        display: 'system' as const,
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'Lengrvis Code installation failed', {
        display: 'system' as const,
      });
    }
  }, [state, onDone]);
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && (
        <Text color="claude">Installing Lengrvis Code native build {state.version}...</Text>
      )}
        ...
        <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Lengrvis Code successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Lengrvis Code native build',
