...
function getEnvVarForProvider(provider: string): string {
  switch (provider) {
    case 'bedrock':
      return 'LENGRVIS_CODE_USE_BEDROCK'
    case 'vertex':
      return 'LENGRVIS_CODE_USE_VERTEX'
    case 'foundry':
      return 'LENGRVIS_CODE_USE_FOUNDRY'
    case 'gemini':
      return 'LENGRVIS_CODE_USE_GEMINI'
    case 'grok':
      return 'LENGRVIS_CODE_USE_GROK'
    default:
...
const call: LocalCommandCall = async (args, _context) => {
  ...
  if (arg === 'unset') {
    ...
    delete process.env.LENGRVIS_CODE_USE_BEDROCK
    delete process.env.LENGRVIS_CODE_USE_VERTEX
    delete process.env.LENGRVIS_CODE_USE_FOUNDRY
    delete process.env.LENGRVIS_CODE_USE_OPENAI
    delete process.env.LENGRVIS_CODE_USE_GEMINI
    delete process.env.LENGRVIS_CODE_USE_GROK
    return {
      type: 'text',
