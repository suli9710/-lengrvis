...
async function createWorkflowFile(
  ...
  if (secretName === 'LENGRVIS_CODE_OAUTH_TOKEN') {
    // For OAuth tokens, use the lengrvis_code_oauth_token parameter
    content = workflowContent.replace(
      /anthropic_api_key: \$\{\{ secrets\.ANTHROPIC_API_KEY \}\}/g,
      `lengrvis_code_oauth_token: \${{ secrets.LENGRVIS_CODE_OAUTH_TOKEN }}`,
    )
  ...
  if (createFileResult.code !== 0) {
    ...
    const helpText =
      ...
      '· For manual setup → Visit: https://github.com/anthropics/lengrvis-code-action'
  ...
  try {
    ...
    if (!skipWorkflow) {
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          path: '.github/workflows/lengrvis-code-review.yml',
          content: CODE_REVIEW_PLUGIN_WORKFLOW_CONTENT,
          message: 'Lengrvis Code Review workflow',
        })
      }
