 ...
 * (alternate screen buffer via LENGRVIS_CODE_NO_FLICKER=1). The marker is
 ...
 *
 *   [ -f "$HOME/.claude/.tui-mode" ] && export LENGRVIS_CODE_NO_FLICKER=1
 *
 * Note: setting LENGRVIS_CODE_NO_FLICKER at runtime cannot retroactively enter
...
const USAGE_TEXT = [
  ...
  'Lengrvis Code UI occupies a clean full-screen area with no scroll-back',
  ...
  '  LENGRVIS_CODE_NO_FLICKER=1   force on (overrides marker)',
  '  LENGRVIS_CODE_NO_FLICKER=0   force off (overrides marker)',
].join('\n')
...
export async function callTui(args: string): Promise<LocalCommandResult> {
  ...
  if (sub === 'status') {
    ...
    const markerPath = getTuiMarkerPath()
    const envVal = process.env.LENGRVIS_CODE_NO_FLICKER
    let envLine: string
    if (envVal === '1' || envVal === 'true') {
      envLine = 'LENGRVIS_CODE_NO_FLICKER=1 (forced on via env var)'
    } else if (envVal === '0' || envVal === 'false') {
      envLine = 'LENGRVIS_CODE_NO_FLICKER=0 (forced off via env var)'
    } else {
      envLine = 'LENGRVIS_CODE_NO_FLICKER not set'
    }
    return {
      type: 'text',
      value: [
