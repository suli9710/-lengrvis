...
beforeEach(() => {
  ...
  origEnv.LENGRVIS_CODE_NO_FLICKER = process.env.LENGRVIS_CODE_NO_FLICKER
  delete process.env.LENGRVIS_CODE_NO_FLICKER
})
...
afterEach(() => {
  ...
  // Restore env vars
  if (origEnv.LENGRVIS_CODE_NO_FLICKER === undefined) {
    delete process.env.LENGRVIS_CODE_NO_FLICKER
  } else {
    process.env.LENGRVIS_CODE_NO_FLICKER = origEnv.LENGRVIS_CODE_NO_FLICKER
  }
})
...
describe('tui status env var display', () => {
  test('shows forced-on when LENGRVIS_CODE_NO_FLICKER=1', async () => {
    process.env.LENGRVIS_CODE_NO_FLICKER = '1'
    const result = await invokeCmd('status')
    expect(result.value).toContain('forced on via env var')
    delete process.env.LENGRVIS_CODE_NO_FLICKER
  })

  test('shows forced-off when LENGRVIS_CODE_NO_FLICKER=0', async () => {
    process.env.LENGRVIS_CODE_NO_FLICKER = '0'
    const result = await invokeCmd('status')
