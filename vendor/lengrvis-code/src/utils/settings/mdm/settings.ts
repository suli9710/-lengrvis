/**
 * MDM (Mobile Device Management) profile enforcement for Lengrvis Code managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.anthropic.lengrviscode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\LengrvisCode` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\LengrvisCode` (user-writable, lowest priority)
 * - Linux: No MDM equivalent (uses /etc/lengrvis-code/managed-settings.json instead)
 *
