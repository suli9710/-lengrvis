 ...
 * When LENGRVIS_CODE_PLUGIN_USE_ZIP_CACHE is enabled and LENGRVIS_CODE_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.LENGRVIS_CODE_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires LENGRVIS_CODE_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.LENGRVIS_CODE_PLUGIN_CACHE_DIR
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/lengrvis-code#13893
  try {
