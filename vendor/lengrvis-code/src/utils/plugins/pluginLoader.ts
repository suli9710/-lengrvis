 ...
 *
 * This module is responsible for discovering, loading, and validating Lengrvis Code plugins
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.LENGRVIS_CODE_REMOTE)
 ...
 * LENGRVIS_CODE_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
 ...
 *
 * LENGRVIS_CODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getLengrvisCodeMcpConfigs()/getInitialSettings().agent run
