...
/**
 * Generates Lengrvis Code specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getLengrvisCodeSnapshotContent(): Promise<string> {
...
async function getSnapshotScript(
  ...
  // Generate the user content and Lengrvis Code content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const lengrvisCodeContent = await getLengrvisCodeSnapshotContent()
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Lengrvis Code defaults only`,
        )
      }
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.LENGRVIS_CODE_DONT_INHERIT_ENV
              ? {}
