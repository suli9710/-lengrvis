...
export const getMTLSConfig = memoize((): MTLSConfig | undefined => {
  ...
  if (process.env.LENGRVIS_CODE_CLIENT_CERT) {
    try {
      config.cert = getFsImplementation().readFileSync(
        process.env.LENGRVIS_CODE_CLIENT_CERT,
        { encoding: 'utf8' },
      )
      logForDebugging(
        'mTLS: Loaded client certificate from LENGRVIS_CODE_CLIENT_CERT',
      )
    } catch (error) {
  ...
  // Client key
  if (process.env.LENGRVIS_CODE_CLIENT_KEY) {
    try {
      config.key = getFsImplementation().readFileSync(
        process.env.LENGRVIS_CODE_CLIENT_KEY,
        { encoding: 'utf8' },
      )
      logForDebugging('mTLS: Loaded client key from LENGRVIS_CODE_CLIENT_KEY')
    } catch (error) {
  ...
  // Key passphrase
  if (process.env.LENGRVIS_CODE_CLIENT_KEY_PASSPHRASE) {
    config.passphrase = process.env.LENGRVIS_CODE_CLIENT_KEY_PASSPHRASE
