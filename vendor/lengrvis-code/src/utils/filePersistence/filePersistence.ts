 ...
 * - Checks environment kind (LENGRVIS_CODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires LENGRVIS_CODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.LENGRVIS_CODE_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but LENGRVIS_CODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
 ...
 * and LENGRVIS_CODE_REMOTE_SESSION_ID.
 * This ensures only public-api/sessions users trigger file persistence,
 * not normal Lengrvis Code CLI users.
 */
