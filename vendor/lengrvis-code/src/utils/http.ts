...
import { getLengrvisCodeUserAgent } from './userAgent.js'
...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.LENGRVIS_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.LENGRVIS_CODE_ENTRYPOINT) {
    parts.push(process.env.LENGRVIS_CODE_ENTRYPOINT)
  }
  ...
  return `lengrvis-code/${MACRO.VERSION}${suffix}`
}
...
// operators match in robots.txt); the lengrvis-code suffix lets them distinguish
// local CLI traffic from claude.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `Claude-User (${getLengrvisCodeUserAgent()}; +https://support.anthropic.com/)`
}
