/**
 * UDS Messaging Layer — Unix Domain Socket IPC for Lengrvis Code instances.
 *
...
export function getDefaultUdsSocketPath(): string {
  ...
  if (process.platform === 'win32') {
    defaultSocketPath = `\\\\.\\pipe\\lengrvis-code-${process.pid}-${nonce}`
 ...
 *
 * Exports `LENGRVIS_CODE_MESSAGING_SOCKET` into `process.env` so child
...
export async function startUdsMessaging(
  ...
  try {
    ...
    process.env.LENGRVIS_CODE_MESSAGING_SOCKET = path
    ...
    if (exportedSocketEnv) {
      delete process.env.LENGRVIS_CODE_MESSAGING_SOCKET
    }
    socketPath = null
