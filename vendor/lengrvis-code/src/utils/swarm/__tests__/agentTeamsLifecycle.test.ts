...
describe('Agent Teams lifecycle', () => {
  test('runs TeamCreate -> spawn -> TaskUpdate -> SendMessage -> TeamDelete', async () => {
    const { TeamCreateTool } = await import(
      '@lengrvis-code-best/builtin-tools/tools/TeamCreateTool/TeamCreateTool.js'
    )
    const { spawnTeammate } = await import(
      '@lengrvis-code-best/builtin-tools/tools/shared/spawnMultiAgent.js'
    )
    const { TaskCreateTool } = await import(
      '@lengrvis-code-best/builtin-tools/tools/TaskCreateTool/TaskCreateTool.js'
    )
    const { TaskUpdateTool } = await import(
      '@lengrvis-code-best/builtin-tools/tools/TaskUpdateTool/TaskUpdateTool.js'
    )
    const { SendMessageTool } = await import(
      '@lengrvis-code-best/builtin-tools/tools/SendMessageTool/SendMessageTool.js'
    )
    const { TeamDeleteTool } = await import(
      '@lengrvis-code-best/builtin-tools/tools/TeamDeleteTool/TeamDeleteTool.js'
    )
