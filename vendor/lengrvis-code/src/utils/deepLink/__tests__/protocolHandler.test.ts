...
const mockParseDeepLink = mock((uri: string) => {
  ...
  return { query: 'hello', cwd: 'E:/Source_code/Lengrvis-code-bast-test' }
})
...
mock.module('../registerProtocol.js', () => ({
  MACOS_BUNDLE_ID: 'com.anthropic.lengrvis-code-url-handler',
}))
...
const originalUrlEvent = process.env.LENGRVIS_CODE_URL_EVENT

beforeEach(() => {
  mockParseDeepLink.mockClear()
  mockLaunchInTerminal.mockClear()
  process.env.__CFBundleIdentifier = undefined
  delete process.env.LENGRVIS_CODE_URL_EVENT
})
...
afterEach(() => {
  ...
  if (originalUrlEvent === undefined) {
    ...
    process.env.LENGRVIS_CODE_URL_EVENT = originalUrlEvent
  }
})
...
describe('handleUrlSchemeLaunch', () => {
  ...
  test('returns null for a matching bundle id when no URL event arrives', async () => {
    process.env.__CFBundleIdentifier = 'com.anthropic.lengrvis-code-url-handler'
  ...
  test('handles a URL event after waiting for url-handler-napi', async () => {
    process.env.__CFBundleIdentifier = 'com.anthropic.lengrvis-code-url-handler'
    process.env.LENGRVIS_CODE_URL_EVENT = 'claude-cli://prompt?q=hello'
