...
export function setShellIfWindows(): void {
  if (getPlatform() === 'windows') {
    ...
    process.env.LENGRVIS_CODE_GIT_BASH_PATH = gitBashPath
...
export const findGitBashPath = memoize((): string => {
  if (process.env.LENGRVIS_CODE_GIT_BASH_PATH) {
    if (checkPathExists(process.env.LENGRVIS_CODE_GIT_BASH_PATH)) {
      return process.env.LENGRVIS_CODE_GIT_BASH_PATH
    }
    console.error(
      `Lengrvis Code was unable to find LENGRVIS_CODE_GIT_BASH_PATH path "${process.env.LENGRVIS_CODE_GIT_BASH_PATH}"`,
    )
  ...
  console.error(
    'Lengrvis Code on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: LENGRVIS_CODE_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
  // eslint-disable-next-line custom-rules/no-process-exit
  process.exit(1)
})
