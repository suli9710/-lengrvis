...
function getEntrypoint(): string | undefined {
  return process.env.LENGRVIS_CODE_ENTRYPOINT
}
...
class Project {
   ...
   * LENGRVIS_CODE_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.LENGRVIS_CODE_SKIP_PROMPT_HISTORY)
    )
  }
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.LENGRVIS_CODE_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // LENGRVIS_CODE_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.LENGRVIS_CODE_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.LENGRVIS_CODE_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
