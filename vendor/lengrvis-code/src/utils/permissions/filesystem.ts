...
import { isAgentMemoryPath } from '@lengrvis-code-best/builtin-tools/tools/AgentTool/agentMemory.js'
import {
  CLAUDE_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_CLAUDE_FOLDER_PERMISSION_PATTERN,
} from '@lengrvis-code-best/builtin-tools/tools/FileEditTool/constants.js'
...
// Always ask when Lengrvis Code tries to edit its own config files
 ...
 * This is a per-user temporary directory used by Lengrvis Code for all temp files.
 *
...
// and per-turn from BashTool prompt. Inputs (LENGRVIS_CODE_TMPDIR env + platform) are
...
export const getClaudeTempDir = memoize(function getClaudeTempDir(): string {
  const baseTmpDir =
    process.env.LENGRVIS_CODE_TMPDIR ||
 ...
 * - MCP CLI state files (managed internally by Lengrvis Code)
