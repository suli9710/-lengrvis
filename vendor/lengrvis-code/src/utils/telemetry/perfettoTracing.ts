/**
 * Perfetto Tracing for Lengrvis Code (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via LENGRVIS_CODE_PERFETTO_TRACE=1 or LENGRVIS_CODE_PERFETTO_TRACE=<path>
 * 2. Optionally set LENGRVIS_CODE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Lengrvis Code normally
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.LENGRVIS_CODE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if LENGRVIS_CODE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.LENGRVIS_CODE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
