...
function socketPath(label: string): string {
  ...
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\lengrvis-code-test-${suffix}`
  }
  return join(tmpdir(), 'lengrvis-code-test', `${suffix}.sock`)
}
...
describe('UDS inbox retention', () => {
  test('drainInbox returns each pending socket message once', async () => {
    ...
    expect(process.env.LENGRVIS_CODE_MESSAGING_TOKEN).toBeUndefined()
  ...
  test('fails closed and cleans temp files when capability target is occupied', async () => {
    ...
    expect(process.env.LENGRVIS_CODE_MESSAGING_SOCKET).toBeUndefined()
