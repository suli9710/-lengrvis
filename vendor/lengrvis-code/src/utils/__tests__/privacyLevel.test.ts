...
describe('getPrivacyLevel', () => {
  const originalDisableNonessential =
    process.env.LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC
  const originalDisableTelemetry = process.env.DISABLE_TELEMETRY

  afterEach(() => {
    delete process.env.LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC
  ...
  test("returns 'essential-traffic' when LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC is set", () => {
    process.env.LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC = '1'
    delete process.env.DISABLE_TELEMETRY
...
describe('isEssentialTrafficOnly', () => {
  const original = process.env.LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC

  afterEach(() => {
    delete process.env.LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC
    if (original !== undefined)
      process.env.LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC = original
  })
...
describe('getEssentialTrafficOnlyReason', () => {
  ...
  test('returns env var name when restricted', () => {
    ...
    expect(getEssentialTrafficOnlyReason()).toBe(
      'LENGRVIS_CODE_DISABLE_NONESSENTIAL_TRAFFIC',
    )
  })
