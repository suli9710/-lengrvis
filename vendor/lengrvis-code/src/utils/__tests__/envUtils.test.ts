...
describe('isBareMode', () => {
  const saved = process.env.LENGRVIS_CODE_SIMPLE
  const originalArgv = [...process.argv]

  afterEach(() => {
    if (saved === undefined) delete process.env.LENGRVIS_CODE_SIMPLE
    else process.env.LENGRVIS_CODE_SIMPLE = saved
    process.argv.length = 0
    process.argv.push(...originalArgv)
  })

  test('returns true when LENGRVIS_CODE_SIMPLE=1', () => {
  ...
  test('returns false when neither set', () => {
    delete process.env.LENGRVIS_CODE_SIMPLE
