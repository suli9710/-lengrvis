/**
 * Lengrvis Code hints protocol.
 *
 * CLIs and SDKs running under Lengrvis Code can emit a self-closing
 * `<lengrvis-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/lengrvis-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type LengrvisCodeHintType = 'plugin'

export type LengrvisCodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: LengrvisCodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<lengrvis-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractLengrvisCodeHints(
  output: string,
  command: string,
): { hints: LengrvisCodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<lengrvis-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: LengrvisCodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[lengrvisCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[lengrvisCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[lengrvisCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as LengrvisCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: LengrvisCodeHint | null = null
...
export function setPendingHint(hint: LengrvisCodeHint): void {
...
export function getPendingHintSnapshot(): LengrvisCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetLengrvisCodeHintStore(): void {
  pendingHint = null
