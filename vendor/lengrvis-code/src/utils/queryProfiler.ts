 ...
 * pipeline from user input to first token arrival. Enable by setting LENGRVIS_CODE_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.LENGRVIS_CODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set LENGRVIS_CODE_PROFILE_QUERY=1)'
  }
