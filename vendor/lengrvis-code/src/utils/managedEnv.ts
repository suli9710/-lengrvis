 ...
 * LENGRVIS_CODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.LENGRVIS_CODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // flagSettings env applied. Eligibility reads LENGRVIS_CODE_USE_BEDROCK,
  ...
  // when LENGRVIS_CODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * LENGRVIS_CODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
