...
describe('getAPIProvider', () => {
  const envKeys = [
    'LENGRVIS_CODE_USE_GEMINI',
    'LENGRVIS_CODE_USE_BEDROCK',
    'LENGRVIS_CODE_USE_VERTEX',
    'LENGRVIS_CODE_USE_FOUNDRY',
    'LENGRVIS_CODE_USE_OPENAI',
    'LENGRVIS_CODE_USE_GROK',
  ] as const
  ...
  test('modelType takes precedence over environment variables', () => {
    process.env.LENGRVIS_CODE_USE_BEDROCK = '1'
    expect(getAPIProvider({ modelType: 'gemini' })).toBe('gemini')
  })

  test('returns "gemini" when LENGRVIS_CODE_USE_GEMINI is set', () => {
    process.env.LENGRVIS_CODE_USE_GEMINI = '1'
    expect(getAPIProvider({})).toBe('gemini')
  })

  test('returns "bedrock" when LENGRVIS_CODE_USE_BEDROCK is set', () => {
    process.env.LENGRVIS_CODE_USE_BEDROCK = '1'
    expect(getAPIProvider({})).toBe('bedrock')
  })

  test('returns "vertex" when LENGRVIS_CODE_USE_VERTEX is set', () => {
    process.env.LENGRVIS_CODE_USE_VERTEX = '1'
    expect(getAPIProvider({})).toBe('vertex')
  })

  test('returns "foundry" when LENGRVIS_CODE_USE_FOUNDRY is set', () => {
    process.env.LENGRVIS_CODE_USE_FOUNDRY = '1'
  ...
  test('"true" is truthy', () => {
    process.env.LENGRVIS_CODE_USE_BEDROCK = 'true'
    expect(getAPIProvider({})).toBe('bedrock')
  })

  test('"0" is not truthy', () => {
    process.env.LENGRVIS_CODE_USE_BEDROCK = '0'
    expect(getAPIProvider({})).toBe('firstParty')
  })

  test('empty string is not truthy', () => {
    process.env.LENGRVIS_CODE_USE_BEDROCK = ''
    expect(getAPIProvider({})).toBe('firstParty')
  })
})
