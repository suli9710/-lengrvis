...
function isManagedOAuthContext(): boolean {
  return (
    isEnvTruthy(process.env.LENGRVIS_CODE_REMOTE) ||
    process.env.LENGRVIS_CODE_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // local auth-injecting proxy. The launcher sets LENGRVIS_CODE_OAUTH_TOKEN as a
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.LENGRVIS_CODE_OAUTH_TOKEN
  }

  const settings = getSettings_DEPRECATED() || {}
  const is3P =
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_BEDROCK) ||
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_VERTEX) ||
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_FOUNDRY) ||
  ...
  const hasExternalAuthToken =
    ...
    apiKeyHelper ||
    process.env.LENGRVIS_CODE_API_KEY_FILE_DESCRIPTOR
...
export function getAuthTokenSource() {
  ...
  if (process.env.LENGRVIS_CODE_OAUTH_TOKEN) {
    return { source: 'LENGRVIS_CODE_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.LENGRVIS_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'LENGRVIS_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      !apiKeyEnv &&
      !process.env.LENGRVIS_CODE_OAUTH_TOKEN &&
      !process.env.LENGRVIS_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'ANTHROPIC_API_KEY or LENGRVIS_CODE_OAUTH_TOKEN env var is required',
      )
    }
 ...
 * Uses LENGRVIS_CODE_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.LENGRVIS_CODE_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found LENGRVIS_CODE_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.LENGRVIS_CODE_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.LENGRVIS_CODE_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
 ...
 *
 * KEEP IN SYNC with providers.ts — when a new LENGRVIS_CODE_USE_* env var is
...
export function isUsing3PServices(): boolean {
  return !!(
    ...
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_VERTEX) ||
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_FOUNDRY) ||
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_OPENAI) ||
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_GEMINI) ||
    isEnvTruthy(process.env.LENGRVIS_CODE_USE_GROK)
  )
}
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.LENGRVIS_CODE_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'LENGRVIS_CODE_OAUTH_TOKEN' ||
    authTokenSource === 'LENGRVIS_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  const isEnvVarToken =
    source === 'LENGRVIS_CODE_OAUTH_TOKEN' ||
    source === 'LENGRVIS_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'LENGRVIS_CODE_OAUTH_TOKEN'
        ? 'LENGRVIS_CODE_OAUTH_TOKEN'
        : 'LENGRVIS_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
