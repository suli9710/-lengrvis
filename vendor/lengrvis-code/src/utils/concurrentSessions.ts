...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.LENGRVIS_CODE_SESSION_KIND
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.LENGRVIS_CODE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.LENGRVIS_CODE_SESSION_NAME,
              logPath: process.env.LENGRVIS_CODE_SESSION_LOG,
              agent: process.env.LENGRVIS_CODE_AGENT,
            }
          : {}),
      }),
    )
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See anthropics/lengrvis-code#34210.
