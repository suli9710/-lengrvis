 ...
 * See: https://github.com/anthropics/lengrvis-code/issues/13893
 *
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    // (anthropics/lengrvis-code#29599, #28182, #26992).
    //
 ...
 * 3. Can be explicitly enabled via LENGRVIS_CODE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
...
> {
  ...
  // - Fallback: native Node.js (when LENGRVIS_CODE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.LENGRVIS_CODE_USE_NATIVE_FILE_SEARCH)
