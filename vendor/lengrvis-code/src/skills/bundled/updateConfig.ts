...
\`\`\`
Plugin syntax: \`plugin-name@source\` where source is \`lengrvis-code-marketplace\`, \`claude-plugins-official\`, or \`builtin\`.
...
Hooks run commands at specific points in Lengrvis Code's lifecycle.
...
Modify Lengrvis Code configuration by updating settings.json files.
