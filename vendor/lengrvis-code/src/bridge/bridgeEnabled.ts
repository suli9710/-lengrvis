 ...
 * (setup-token, LENGRVIS_CODE_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    ...
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `claude setup-token` or LENGRVIS_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `claude auth login` to use Remote Control.'
    }
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Lengrvis Code (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`claude update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.LENGRVIS_CODE_CCR_MIRROR) ||
