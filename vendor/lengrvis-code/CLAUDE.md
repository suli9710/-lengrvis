...
This is a **reverse-engineered / decompiled** version of Anthropic's official Lengrvis Code CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced — **`bun run precheck` 必须零错误通过**（包含 typecheck + lint fix + test）。
...
- **`packages/builtin-tools/src/tools/`** — 60 个工具目录（含 shared/testing 等工具目录），通过 `@lengrvis-code-best/builtin-tools` 包导出。主要分类：
